...
touch ${WORK_DIR}/failed_tests_${CI_PIPELINE_ID}_${CMSSW_release}
touch ${WORK_DIR}/retry_tests_${CI_PIPELINE_ID}_${CMSSW_release}
...
mv ${WORK_DIR}/retry_tests_${CI_PIPELINE_ID}_${CMSSW_release} ${WORK_DIR}/old_retry_tests_${CI_PIPELINE_ID}_${CMSSW_release}
touch ${WORK_DIR}/retry_tests_${CI_PIPELINE_ID}_${CMSSW_release}
...
if [ -f "${WORK_DIR}/submitted_tasks_CCV_${CI_PIPELINE_ID}_${CMSSW_release}" ]; then
  while read task; do
    ...
    testOK=0; testFail=0
    grep -q ${test_to_execute}  ${WORK_DIR}/successful_tests_${CI_PIPELINE_ID}_${CMSSW_release} && testOK=1 || true
    grep -q ${test_to_execute}  ${WORK_DIR}/failed_tests_${CI_PIPELINE_ID}_${CMSSW_release}  && testFail=1 || true
    if [ $testOK -eq 1 ]; then
  ...
  echo ""
  cat  ${WORK_DIR}/successful_tests_${CI_PIPELINE_ID}_${CMSSW_release} | cut -d ' ' -f 2 || true
  echo "======================================================="
  echo "==== TESTS TO BE RUN AGAIN "
  echo ""
  cat  ${WORK_DIR}/retry_tests_${CI_PIPELINE_ID}_${CMSSW_release} 2>/dev/null | cut -d ' ' -f 2 || true
  echo "======================================================="
  echo "==== FAILED TESTS "
  echo ""
  cat  ${WORK_DIR}/failed_tests_${CI_PIPELINE_ID}_${CMSSW_release} | cut -d ' ' -f 2 || true
  echo "======================================================="
